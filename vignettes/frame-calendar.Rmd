...
Furthermore, the argument `y` is possible to take multiple variable names that need to be quoted. It's difficult to spot the differences between two sensors from the overlaying plot shown earlier. The rectangular glyphs arranged on a "weekly" calendar are plotted to illustrate the multiple `y`s use and the differences between sensors. The long data format is firstly converted to the wide format using `tidyr::spread()` [@wickham2014tidy]. These two sensors are variables rather than values now, and hence can be passed to `y`. 
...
sensors_wide_calendar <- two_sensors_wide %>% 
  frame_calendar(x = Time, y = c("Centre", "Lonsdale"), date = Date, 
    calendar = "weekly" )
