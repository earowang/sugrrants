...
A snapshot of foot traffic as a whole in the downtown area can be taken (or summarised) using a boxplot too.

```{r ped-box, fig.height = 5}
ped_calendar <- pedestrian17 %>% 
  filter(Month == "January") %>%
  frame_calendar(x = Time, y = Hourly_Counts, date = Date)
p_boxplot <- ped_calendar %>% 
