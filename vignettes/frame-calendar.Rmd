...
Furthermore, the argument `y` can take multiple variable names in combination with `vars()`. It's difficult to spot the differences between two sensors from the overlaying plot shown earlier. The rectangular glyphs arranged on a "weekly" calendar are plotted to illustrate the multiple `y`s use and the differences between sensors. The long data format is firstly converted to the wide format using `tidyr::spread()` [@wickham2014tidy]. These two sensors are variables rather than values now, and hence can be passed to `y`.
